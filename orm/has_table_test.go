...
var _ = Describe("HasTable", func() {
	It("has table", func() {
		...
		Expect(string(b)).To(Equal("SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'has_table_models');"))
	})
	It("has table with custom name", func() {
		...
		b, err := hasTableQuery{q: q}.AppendQuery(nil)
		Expect(err).NotTo(HaveOccurred())
		Expect(string(b)).To(Equal("SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'custom_name');"))
	})
})
