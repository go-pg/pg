...
var _ = Describe("Join", func() {
	...
	It("supports belongs to", func() {
		...
		s := selectQueryString(q)
		Expect(s).To(Equal(`SELECT "JoinTest"."id", "JoinTest"."has_one_id", "belongs_to"."id" AS "belongs_to__id", "belongs_to"."join_test_id" AS "belongs_to__join_test_id" FROM "JoinTest" LEFT JOIN "BelongsTo" AS "belongs_to" ON "belongs_to"."join_test_id" = "JoinTest"."id"`))
	})
})
