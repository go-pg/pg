...
func (db *DB) Query(coll, query interface{}, params ...interface{}) (res types.Result, err error) {
	backoff := defaultBackoff
	for i := 0; i < 3; i++ {
		var cn *conn
...
func assertOneAffected(res types.Result, model *singleElementCollection) (types.Result, error) {
